#!/usr/bin/env python3
"""
Trigger Test Workflow 2 (poc_workflow - different service order)
Simple script to trigger the poc_workflow from test_workflow2.json
Automatically registers the workflow if not already registered
Service order: Email → Phone → Airflow
"""
...
WORKFLOW_FILE = os.path.join(os.path.dirname(__file__), "..", "workflows", "test_workflow2.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Trigger Test Workflow 2')
    parser.add_argument('--update', action='store_true',
                       help='Force re-register/update the workflow even if already registered')
    args = parser.parse_args()
    
    print("=" * 60)
    print(f"Test Workflow 2 Manager (poc_workflow)")
    print("Service Order: Email → Phone → Airflow")
    print("=" * 60)
    ...
    print(f"   - Service execution order: Email → Phone → Airflow")
    print()
